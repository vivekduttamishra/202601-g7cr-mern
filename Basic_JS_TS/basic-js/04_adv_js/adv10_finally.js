...
function fetchPredictionFromDb(luckyNumber){
    ...
    if(isNaN(luckyNumber))
        throw new LuckError('Invalid Number',luckyNumber)

    if(luckyNumber===0)
        throw new NoLuckError()

    if(luckyNumber<0)
        throw new TooMuchNegativityError('Too Much Negativity', luckyNumber)

    if(luckyNumber>=predictions.length)
        throw new LuckError('Too Ambitious: choose between 1 and 5', luckyNumber)
...
function getPrediction(luckyNumber){
    try{
        ...
        console.log('Finally in finally')
    }
    //console.log('end of getPrediction')
}
...
function fortuneTeller(luckyNumber){
    try{
        ...
        console.log(`Your fortune: ${err.message}`)
    }
}
